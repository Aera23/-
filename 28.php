<?php
if($_COOKIE['name'] == file_get_contents("unlock.txt") && $_COOKIE['name']!=""){exit('<meta http-equiv="refresh" content="4"><mark>You unlocked a clock:<br><br>'.date("H:i:s").'</mark>');}
...
if($_GET['b']=="d"){$file=file("1id8sjl.txt");for($i=count($file)-1;$i>=max(0,count($file)-16);$i--){$t.=$file[$i];}$limit = substr_count($t,htmlspecialchars(str_ireplace("Aera348","Aera23",$_REQUEST['name']))." ");  if($limit>7 && ($_POST['comment']!="" || $_POST['rpl']!="")){file_put_contents("9u9dyi","|m".time(),FILE_APPEND);exit("<mark>! <a href='28.php?b=d&col=".base64_encode(htmlspecialchars($_POST['col']))."'>Clear then wait for reply</a></mark>");}}
...
'.$hide.'</style></form><br>Debug: ';print_r($_POST);echo'</html>';exit;}
...
<p style="color:#8e8">Emojis and commands: :alien: :books: :tree: :book: :kiss: :dog: :fire: :ice: :cash:<br><i style="color:#f88">NEW: :egg: :taco: :eggplant: :rocket: :ghost: :elephant: :snail: :lips: :pumpkin: :xmas: :bird: :lovedock: :docking: :spacedock: :miii:</i><br><span style="color:#e6e">:shrug: /shrug :goat: :rainbow: /wow /symbols :cat: :music: :shroom: /me, /dice, <mark style="color:red;background:#400"><3</mark>, <mark>:)</mark>, <mark>:3)</mark> , <mark>XD</mark></a>, :panda: :fox: :duck:</span></p><b><a href="/solr/ncs-'.mt_rand(0,11).'.mp3" target="_blank" download>Save some NCS Music</a>|<a href="22.php" target="aa">Random colours 👀</a> | <a href="colour.php" target="_blank">Latency 👀</a> | <a href="28.php?a=a" target="aa">Reload messages</a> | <a href="files.php" target="_blank">File upload 👀</a>';if($config[5]==1){echo'<br><br>
...
</div><div class="hidden"><p style="display:inline">Please keep things non-toxic (Reload if needed, Bookmark too) <mark>:)</mark></p></div><iframe class="v" style="float:right;width:';
...
echo'%;height:10em"></iframe><br><b style="color:#4f4">Welcome to '.$phrase.' :)</b>
