...
exit('<mark>Command run: '.htmlspecialchars($_GET['d']).' [1: normal captcha, 2: spaced out captcha, 3: disable API, 4: enable API, 5: disable server side beep, 6 enable it]</mark>');
}
$ok='2';
#CAPTCHA
function mr($gt){exit('<meta http-equiv="refresh" content="0 g3.php?next=28.ph'.$gt.'"><mark>CAPTCHA redirect</mark>');}
if(isset($_COOKIE['crc'])&&$_COOKIE['o']<time()&&crc32(base64_encode($_SERVER['HTTP_USER_AGENT']."127.0.0.1".$_COOKIE['o']))==$_COOKIE['crc']){$ok=1;}
...
if(crc()==983){$_POST['name']="ENCH4127";$safe="ENCHANTRESS";}
...
#Benchmarking
$start=number_format(microtime(true),10,'.','');
...
$arr[]= $p.$name.'</td><td style="text-align:center">'.str_replace("3 ", "31 ",str_replace(date("jS"),"",str_replace(date("m-"),"",$time.'<span class="x"></td><td style="text-align:right"><a href="28.php?f='.$name.'.visit&i='.max($qq-600,0).'" target="_blank">'.number_format($qq))))."</a></td>
</tr>";}else{break;}}
$tt='';$z=0;foreach($arr as $str){if($z<11){$tt=$tt.$str;}$z++;}
echo"<!DOCTYPE html><html><meta http-equiv='refresh' content='$sec 28.php?b=b&o=$e'>";
if(isset($_GET['o'])&&$_GET['o']!=$e&&isset($_COOKIE['ina'])&&$_GET['b']=='b'){$phrase='-';include('g3.php');gt();echo'<mark style="display:inline">*</mark>';}
...
#End bench, reuse var
$end=number_format(microtime(true),10,'.','');
$z=($end-$start)*1000;
exit($e.' (last 30s)</span><br><table>'.$tt.'</table></center><mark>'.$z.'</mark></html>');}
...
$x=['hild p','teleg','t.me','h i l d','nigge'];#Filters trimmed, add your own!
foreach($x as $y){
...
#Nick protect
if(strpos($_POST['comment'],"/np ")===0){$p=htmlspecialchars(str_replace("/np ","",str_replace('|','',nl2br($_POST['comment']))));
...
#Ban
if(strpos($_POST['comment'],"/k ")===0){$cf=fopen($hpu,"w") or die($m);$e=htmlspecialchars(str_replace("/k ","",$_POST['comment']));fwrite($cf, $e) or die("<a href='28.php?b=d'><mark>Can't write</mark></a>");fclose($cf);$z=4;echo"<mark>Banned $e</mark>";file_put_contents("whokicked.txt",htmlspecialchars($_COOKIE['name']));}
#AntiRaid
...
#Unban
if(strpos($_POST['comment'],"/uk ")===0){unlink($hpu) or die($m);$e=htmlspecialchars(str_replace("/uk ","",$_POST['comment']));if(file_exists($e.'.visit.ivisit')){rename($e.'.visit.ivisit',$e.'.visit');};$z=4;echo"<mark>Unbanned ".safe($e)."</mark>";}
...
input{padding:0.5em;background:#450;border:3px solid #300;color:#fff !important;font-family:'.htmlspecialchars($_POST['f']??'sans-serif').'}input:hover{padding:0.5em;background:#560;border:3px solid #500}
...
echo'.r{color:#6ff}.g{width:15em;margin-right:8em;margin-left:0.5em;background:#004;padding:0.4em;border-radius:10px}
/*Clock :)*/
@keyframes t{
...
.l,.r{font-family:monospace}.l{font-size:1.1em}.l span{height:15px;width:15px;border:1px solid grey;background:'.(htmlspecialchars($_COOKIE['col'])?:'#8f8').';display:inline-block;position:relative;margin-top:5px;}.l input{opacity:0.01}
...
</style><input id="show" type="checkbox"><label for="show"><u>'.$phrase.' Help<b></b></u> | <a href="music.php" target="_blank">🎵 Music</a> | <a href="22.php" target="_blank">🎨 Hotbit colours</a> | <a href="colour.php" target="_blank">🚅 SpeedTest</a> | <a href="files.php" target="_blank">⬆️ Files</a> | <a href="add.php" target="_blank">🏜️, Pyramid Puzzle</a>';
...
if(strpos($a,$e)!==false&&$e!=""){$b.=$btn.'v" title="Edit message">'.(strpos($_SERVER['HTTP_USER_AGENT'],'(Windows NT 10.0; Win64; x64; rv:127.0)')||strpos($_SERVER['HTTP_USER_AGENT'],'Edg')?'':'_').'🖌️'.$z;}
elseif($i%2==0){
...
$e='><mark style="display:inline;background:#000;color:#afa;font-family:monospace;font-size:14px">'.file_get_contents("load.txt").'</mark>';}
else{$e='>';}
...
echo'<title>'.$phrase.'</title><meta http-equiv="refresh" content="'.$sec.' ?a=a&t='.$time.'"><style>pre{color:#9f9;font-family:sans-serif;background:inherit;white-space:pre-wrap}a{color:#07f}a:hover{color:#44f}form{display:inline}.y,.v{background:'.$cfi.';}.v{padding:0.2em;}.w,.z{background:#000;color:'.$cfi.'}.v:hover{border:3px solid #0f0}.w:hover{border:3px solid #f80}.y:hover{border:3px solid #f22}.z:hover{border:3px solid #0ff}button{border-radius:8px;border: 3px solid #000;font-family:monospace}.ref{animation:ref 4s infinite;animation-delay:'.($sec+6).'s;background:radial-gradient(#000014, #001400, #140000)}@keyframes ref{0%{background:radial-gradient(#000014, #001400, #140000)}50%{background:#f22}100%{background:radial-gradient(#000014, #001400, #140000)}}
...
echo'<pre style="word-wrap:break-word;line-height:1.7;margin-top:-0.1em"><a href="canary.txt" target="_blank" style="color:#9f9">'.htmlspecialchars_decode($config[3]).'</a><br>';
...
#Prep for caching, change '' to your ramcache if existing
if(is_dir('Music')){
